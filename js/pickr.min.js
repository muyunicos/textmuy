		...
		class L {
			...
			_bindEvents() {
				const {
					...
					// Patch close picker with Enter key 
					(t.key === "Enter" && this.hide())
				})), i(t.interaction.result, ["focus", "blur"], (t => {
					// Patch select text input
					//t.target.select();
					t.target.setSelectionRange(1, t.target.value.length);
				...
				if (!e.showAlways) {
					...
					o.push(i(t.button, "click", (() => this.isOpen() ? this.hide() : this.show())), i(document, "keyup", (t => this.isOpen() && (t.key === "Enter" || t.key === n || t.code === n) && this.hide())), i(document, ["touchstart", "mousedown"], (e => {
			...
			_emit(t, ...e) {
				this._eventListener && this._eventListener[t].forEach((t => t(...e, this)))
			}
			on(t, e) {
			...
			addSwatch(t) {
				...
				if (e) {
					const {
						_swatchColors: t,
						_root: o
					} = this, n = A(...e), s = r(`<button type="button" style="--pcr-color: ${n.toRGBA().toString(0)}" title="${n.toHEXA().toString(0)}" aria-label="${this._t("btn:swatch")}"/>`);
