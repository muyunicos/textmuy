...
Lo que el administrador ve en el editor debe ser exactamente lo que aparece en el PDF procesado. La revisión verifica que el motor sin interfaz (render-core, el que consume el plugin al procesar grupos) produzca el mismo resultado visual que el editor para el mismo estilo y texto, respetando el contrato público de render: salida del tamaño exacto definido por el estilo, carga garantizada de la tipografía antes de dibujar, rechazo ante el primer fallo con causa y nunca lotes parciales. **Premisa**: WebGL disponible conforme a la Constitución II (sin WebGL la ruta de API falla por diseño, de modo que la paridad solo es verificable con WebGL presente).
...
- **FR-004**: La revisión MUST verificar los comportamientos degradados documentados y su resultado observable, sin datos falsos ni degradaciones silenciosas: (a) **sin configuración del sistema** → estado de error claro y accionable, cero lecturas a rutas locales del módulo y cero operaciones de guardado; (b) **sin aceleración gráfica en la ruta de API** → fallo con causa explícita, sin resultado degradado ni sustitución silenciosa; (c) **sin internet** → comportamiento documentado de tipografías remotas, sin errores no controlados y sin fuente sustituta en el render.
...
- **FR-006**: Cada punto de contrato verificado MUST quedar registrado como PASS o FAIL, con evidencia del comportamiento observado; no se acepta un punto "sin verificar" al cierre, salvo que dependa de datos del administrador no disponibles (registrado como condicionado); la publicación del artefacto y las correcciones documentales se rigen por FR-011.
...
- **SC-003**: En las verificaciones de paridad del conjunto representativo (premisa: WebGL disponible), el resultado del motor sin interfaz coincide con el del editor en **dimensiones exactas (100% de los casos)** y en **apariencia visible**: comparación lado a lado a 100% y 200% de zoom sin diferencias perceptibles (0 casos con diferencia perceptible), con captura registrada por caso; toda diferencia perceptible se registra como hallazgo con severidad asignada.
