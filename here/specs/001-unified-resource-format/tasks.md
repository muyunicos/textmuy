...
- [X] R008 HECHA (2026-09-12): limpieza fisica del espejo `../uploads/tm/` — 15 archivos eliminados: `fonts/fonts.json.legacy`, `img/catalogo.json`, `img/catalogo.json.legacy`, `img/thumbs.webp` (artefacto del formato anterior; el ThumbEngine actual solo escribe/lee `thumbs/{scope}.webp`), `presets/thumbs/` completa (`presets.json` + `presets.webp` viejos por nombre) y los 9 `.webp` sueltos de presets. Solo quedan los 3 JSON canonicos (`fonts.json` 72 items, `img.json` 128 items, `presets.json` 10 items) + fisicos + `.txm`. Los sprites se regeneran idempotentes via `guardarSprite` al abrir cada galeria.
